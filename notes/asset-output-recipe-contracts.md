# Asset Output Recipe Contracts

Generated by `tools/build_asset_output_recipe_contracts.py` from checked-in `asset-manifests/*.json` and `tools/asset_output_recipe_contracts.py`.

This report is a typed emitter/render/decode coverage map. It contains no ROM-derived payloads; it validates the reproducible recipes that `tools/extract_assets.py` uses when a user supplies a ROM.

## Snapshot

- manifests: `38`
- assets/tables/gaps represented: `2219`
- output recipes: `6175`
- typed output recipe kinds: `13`
- decoder-backed output recipes: `3956`
- renderer-backed output recipes: `3264`
- PNG preview/render recipes: `3264`

## Reproducibility Rules

- Every output recipe kind is known to the typed contract registry.
- Every output path is relative to the configured output root and uses POSIX separators.
- PNG and JSON recipe paths use the extension implied by their typed output kind.
- Renderer dimensions, palette counts, tile columns, and ids use integer fields with bounded values.
- Palette and graphics source references carry rom-range, byte count, SHA-1, and optional earthbound_lzhal compression metadata.

## Recipe Kinds

| Recipe kind | Recipes | Assets | Output type | Decoder | Renderer | Required fields | Example |
| --- | ---: | ---: | --- | --- | --- | --- | --- |
| `earthbound_lzhal` | 466 | 466 | decompressed binary | `earthbound_lzhal` | - | - | `asset.ca.battle_background_gfx_63` -> `ca/battle_bgs/graphics/63.gfx` |
| `earthbound_lzhal_battle_bg_arrangement_png` | 103 | 103 | composed battle-background PNG | `earthbound_lzhal+snes_tilemap+snes_tiles+snes_palette` | `battle_background_arrangement` | `width_tiles`, `height_tiles`, `bpp`, `colors`, `arrangement_id`, `graphics_id`, `palette_id`, `graphics_source`, `palette_source` | `asset.ca.battle_background_arr_37` -> `ca/battle_bgs/arrangements/37_composed_preview.png` |
| `earthbound_lzhal_battle_sprite_png` | 166 | 110 | composed battle-sprite PNG | `earthbound_lzhal+snes_4bpp_tiles+snes_palette` | `battle_sprite` | `width`, `height`, `colors`, `sprite_id`, `palette_id`, `palette_source` | `asset.cd.battle_sprite_107` -> `cd/battle_sprites/107_palette_01_sprite.png` |
| `earthbound_lzhal_snes_4bpp_tiles_palette_png` | 234 | 178 | decompressed palette-applied tile preview PNG | `earthbound_lzhal+snes_4bpp_tiles+snes_palette` | `palette_tile_sheet` | `columns`, `colors`, `palette_source` | `asset.ca.battle_background_gfx_38` -> `ca/battle_bgs/graphics/38_palette_preview.png` |
| `earthbound_lzhal_snes_4bpp_tiles_png` | 237 | 237 | decompressed tile preview PNG | `earthbound_lzhal+snes_4bpp_tiles` | `grayscale_tile_sheet` | `columns` | `asset.ca.battle_background_gfx_63` -> `ca/battle_bgs/graphics/63_4bpp_preview.png` |
| `earthbound_lzhal_snes_palette_json` | 10 | 10 | decompressed SNES palette JSON | `earthbound_lzhal+snes_palette` | - | - | `asset.e1.compressed_palette_unknown` -> `e1/unknown_palette_palette.json` |
| `earthbound_lzhal_snes_palette_swatch_png` | 10 | 10 | decompressed palette swatch PNG | `earthbound_lzhal+snes_palette` | `palette_swatch` | `per_row`, `swatch` | `asset.e1.compressed_palette_unknown` -> `e1/unknown_palette_palette.png` |
| `raw` | 2219 | 2219 | byte-for-byte extract | - | - | - | `asset.ca.battle_background_gfx_63` -> `ca/battle_bgs/graphics/63.gfx.lzhal` |
| `snes_2bpp_tiles_png` | 1 | 1 | tile preview PNG | `snes_2bpp_tiles` | `grayscale_tile_sheet` | `columns` | `asset.debug.cursor_graphics` -> `ef/debug_cursor_preview.png` |
| `snes_4bpp_tiles_palette_png` | 1146 | 1146 | palette-applied tile preview PNG | `snes_4bpp_tiles+snes_palette` | `palette_tile_sheet` | `columns`, `colors`, `palette_source` | `asset.d1.sprite_0000` -> `d1/overworld_sprites/gfx/0000_palette_00_preview.png` |
| `snes_4bpp_tiles_png` | 1151 | 1151 | tile preview PNG | `snes_4bpp_tiles` | `grayscale_tile_sheet` | `columns` | `asset.d1.sprite_0000` -> `d1/overworld_sprites/gfx/0000_4bpp_preview.png` |
| `snes_palette_json` | 216 | 216 | decoded SNES palette JSON | `snes_palette` | - | - | `asset.cb.battle_background_palette_113` -> `cb/battle_bgs/palettes/113_palette.json` |
| `snes_palette_swatch_png` | 216 | 216 | palette swatch PNG | `snes_palette` | `palette_swatch` | `per_row`, `swatch` | `asset.cb.battle_background_palette_113` -> `cb/battle_bgs/palettes/113_palette.png` |

## Family Coverage

| Family | Banks | Assets | Outputs | Decoder-backed | Renderer-backed | PNG previews | Output mix | Renderer mix |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- | --- |
| Battle visual assets | `CA, CB, CC, CD, CE` | 689 | 2117 | 1428 | 886 | 886 | `raw` 689, `earthbound_lzhal` 361, `earthbound_lzhal_snes_4bpp_tiles_palette_png` 234, `earthbound_lzhal_snes_4bpp_tiles_png` 202, `snes_palette_json` 181, +3 more | `palette_tile_sheet` 234, `grayscale_tile_sheet` 202, `palette_swatch` 181, `battle_sprite` 166, `battle_background_arrangement` 103 |
| Mixed asset/table banks | `CF, D0` | 7 | 7 | 0 | 0 | 0 | `raw` 7 | - |
| Overworld sprites | `D1, D2, D3, D4, D5` | 1148 | 3440 | 2292 | 2292 | 2292 | `raw` 1148, `snes_4bpp_tiles_palette_png` 1146, `snes_4bpp_tiles_png` 1146 | `grayscale_tile_sheet` 1146, `palette_tile_sheet` 1146 |
| Map tilesets and runtime tables | `D6, D7, D8, D9, DA, DB, DC, DD, DE, DF` | 130 | 282 | 152 | 55 | 55 | `raw` 130, `earthbound_lzhal` 64, `snes_palette_json` 33, `snes_palette_swatch_png` 33, `earthbound_lzhal_snes_4bpp_tiles_png` 22 | `palette_swatch` 33, `grayscale_tile_sheet` 22 |
| UI, fonts, and town-map assets | `E0, E1` | 69 | 152 | 83 | 30 | 30 | `raw` 69, `earthbound_lzhal` 41, `earthbound_lzhal_snes_4bpp_tiles_png` 13, `earthbound_lzhal_snes_palette_json` 10, `earthbound_lzhal_snes_palette_swatch_png` 10, +3 more | `grayscale_tile_sheet` 18, `palette_swatch` 12 |
| Audio packs | `E2, E3, E4, E5, E6, E7, E8, E9, EA, EB, EC, ED, EE` | 171 | 171 | 0 | 0 | 0 | `raw` 171 | - |
| EF debug and late-tail data | `EF` | 5 | 6 | 1 | 1 | 1 | `raw` 5, `snes_2bpp_tiles_png` 1 | `grayscale_tile_sheet` 1 |

## Typed Output Boundary

- `raw` and `earthbound_lzhal` are reproducible extraction/decode recipes, not semantic flattening of hand-authored payloads.
- Palette-applied and composed preview recipes are fixtures: they bind graphics bytes to explicit palette/graphics source refs with range SHA-1s.
- Audio-pack outputs remain raw by design; this contract only proves byte-stable extraction until a later audio format boundary is chosen.
- Table and raw-gap outputs remain typed as byte-for-byte extraction recipes so source scaffolds keep intentional data blocks intact.
