...
- The pack-1 driver block contains a strong 0x1F indirect table candidate at 0x16C7 that cleanly maps 32 entries onto the E0..FF high-command byte range used by sequence payloads.
- Under that mapping, FF points at driver target 0x1A81, giving the FF terminator review lane a concrete address for the next SPC700 disassembly or execution trace.
- The follow-up FF target review keeps 0x1A81 provisional because the target span is data-like under byte profiling.
...
- disassemble or trace execution at 0x1A81 with live channel state to determine whether FF ends a sequence, returns from EF, stops a channel, or falls into another control path
- capture a short runtime trace around the 0x1F 0x16C7 dispatch for at least one sequence that reaches FF
