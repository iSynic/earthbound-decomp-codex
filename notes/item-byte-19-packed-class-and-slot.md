...
- `C1:9B79..9CDD` is now decoded source in `src/c1/c1_9b79_resolve_equipped_slot_for_item_subtype.asm`.
...
4. uses the result to choose one of the four equipped-reference bytes in the selected character record:
   - `0x00 -> $99FF`
   - `0x04 -> $9A00`
   - `0x08 -> $9A01`
   - `0x0C -> $9A02`
...
- the `C1:9B79` source module now exposes the exact comparison-marker continuation: after writing one `$9A1D` row it jumps back to the existing `C1:9B62` loop until all four active-party rows have been processed
