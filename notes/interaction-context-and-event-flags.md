...
- `C1:0301..042E` is now decoded source in
  `src/c1/c1_0301_get_active_interaction_context_record.asm`.
- `C1:042E..045D` is now decoded source in
  `src/c1/c1_042e_increment_current_text_context_workmem.asm`.
- `C1:045D..0489` is now decoded source in
  `src/c1/c1_045d_install_primary_interaction_context_pointer.asm`.
- `C1:0489..04B5` is now decoded source in
  `src/c1/c1_0489_install_secondary_interaction_context_pointer.asm`.
- `C1:04B5..078D` is now decoded source in
  `src/c1/c1_04b5_get_current_text_context_line_state.asm`, with
  `C1:04EE` covered as the larger window/context allocation and descriptor
  binding tail.
...
- `C1:0324` snapshots context fields `+$17`, `+$1B`, and `+$1F` into `+$21`, `+$25`, and `+$29`
- `C1:0380` restores `+$21`, `+$25`, and `+$29` back into `+$17`, `+$1B`, and `+$1F`
- `C1:03DC` loads the pointer pair at `+$1B` into `$14/$16`
- `C1:040A` loads the pointer pair at `+$17` into `$14/$16`
...
- `C1:04EE` creates or binds a logical window slot to a `$8650` descriptor,
  links it into `$88E0/$88E2`, copies context pointer slots from the previous
  active descriptor, clears the target tile region to blank tile `$0040`, marks
  redraw state through `$9623`, and refreshes window state through `C0:7C5B`
