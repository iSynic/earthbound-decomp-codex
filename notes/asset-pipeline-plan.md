...
- `tools/build_map_fts_format_audit.py` consumes EBDecomp `.fts` exports to write `notes/map-fts-format-audit.json`, a payload-free section map that identifies the 64-character indexed tile rows, variable settings rows, and unresolved 96-character records.
