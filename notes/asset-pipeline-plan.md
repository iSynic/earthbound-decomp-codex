...
- `tools/build_map_fts_animation_settings_contract.py` consumes EBDecomp `.fts` exports to write `notes/map-fts-animation-settings-contract.json`, a structural contract for the variable 290-character tile-animation/settings rows, including row group/slot ownership, fixed block-position profiles, and anchors to the ROM-verified EF animation runtime tables plus DE/DF animation graphics payloads.
