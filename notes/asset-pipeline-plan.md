...
- `tools/build_map_fts_animation_settings_contract.py` consumes EBDecomp `.fts` exports to write `notes/map-fts-animation-settings-contract.json`, a structural contract for the variable 290-character tile-animation/settings rows.
...
- `tools/build_map_collision_attribute_context.py` consumes the scene composition contract, `map_tiles.map`, and direct `.fts` arrangement/collision rows to write `notes/map-collision-attribute-context.json`, a payload-free context audit for the unresolved third arrangement-cell byte.
- `tools/build_map_palette_descriptor_context.py` consumes the same scene composition and `.fts` contracts to write `notes/map-palette-descriptor-context.json`, a payload-free audit of descriptor palette-bit use and candidate six-subpalette fit.
