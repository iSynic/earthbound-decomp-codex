...
Bank `C0` is structurally closed for the source-scaffold phase:

- durable scaffold: `src/c0/bank_c0_helpers_asar.asm`
- range manifest: `build/c0-build-candidate-ranges.json`
- residual map: `notes/c0-source-residual-map.md`
- byte-equivalence report: `notes/c0-byte-equivalence-validation.md`
- handoff: `notes/bank-c0-source-scaffold-handoff.md`
- validation: `C0 byte-equivalence: OK, 504 module(s), 0 mismatch(es).`

The current scaffold is working-name anchored: each interval between known C0
labels is preserved as exact ROM bytes. This is byte closure, not final
semantic source; the next phase is replacing those corridors with decoded
subsystem source while keeping the same validation loop.
