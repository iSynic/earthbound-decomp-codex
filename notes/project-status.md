...
1. `C3`: event/actionscript opcode and operand semantics. The first audit is
   ...
   callback byte-count seeds captured. Eighteen source-form pilots are checked in
   under `src/c3/event_scripts/` and cover `9573` validated bytes across the
   ...
   fade/coordinate path, and position/text-yield path families.
