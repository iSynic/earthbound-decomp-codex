...
1. `C3`: event/actionscript opcode and operand semantics. The first audit is
   ...
   callback byte-count seeds captured. Five source-form pilots are checked in
   under `src/c3/event_scripts/` and cover `3329` validated bytes across the
   movement pulse preset, timed-delivery, service-event movement, and
   service-animation helper/event and presentation/effect families. The next C3
   work is promoting more script families through that pattern, not unknown
   opcode recovery.
