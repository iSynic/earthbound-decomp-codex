...
1. `C3`: event/actionscript opcode and operand semantics. The first audit is
   ...
   callback byte-count seeds captured. Seventeen source-form pilots are checked in
   under `src/c3/event_scripts/` and cover `9305` validated bytes across the
   ...
   follower path, vehicle coordinate path, Boogy Tent/city bus path, and
   position/text-yield path families.
