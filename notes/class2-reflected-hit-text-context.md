...
- `C1:AC4A` and `C1:ACA1` are not just abstract refresh hooks. They now read as counted-copy builders for the live attacker-name and target-name buffers at `$9CD7` and `$9CF5`, using `C0:8EED`, then writing a terminator byte and invalidating `$9658/$965A`.
