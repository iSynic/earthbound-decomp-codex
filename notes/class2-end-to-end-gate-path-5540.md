...
- The front battle-start/status prelude immediately before this controller is decoded source as `C2:4F52..5024` in `src/c2/c2_4f52_display_battle_start_status_messages_prelude.asm`.
- The heavier controller tail `C2:5024..6189` is now source-promoted as two byte-equivalent modules:
  - `src/c2/c2_5024_run_battle_start_candidate_controller_front.asm` (`C2:5024..5AFB`)
  - `src/c2/c2_5afb_run_battle_start_candidate_controller_back.asm` (`C2:5AFB..6189`)
  The split keeps the accumulator-width joins explicit instead of hiding them inside one fragile linear emission.
