...
- `C0:2291` = `TestSecondaryDescriptorLeadingPieceContext`
...
The C4 bank audit now treats the secondary descriptor records as their own address-bearing chunks, so this note also needs explicit record identities rather than only the pointer table.

The machine-readable contract is now generated by `tools/build_secondary_visual_descriptor_contract.py` as `notes/secondary-visual-descriptor-contracts.json`. It records the `C4:2B0D..2B51` pointer table, `16` unique descriptor records, the first duplicated pointer target, and the adjacent non-descriptor ranges before `C4:2F8C`.

The current inventory is:

| Address | Header | Shape Read | Notes |
| --- | --- | --- | --- |
| `C4:2B51` | `01,00` | 1 piece | minimal single-piece descriptor |
| `C4:2B5D` | `02,00` | 2 pieces | narrow two-piece descriptor |
| `C4:2B73` | `02,00` | 2 pieces | wider/symmetric two-piece descriptor |
| `C4:2B89` | `03,00` | 3 pieces | three-piece vertical-ish descriptor |
| `C4:2BA9` | `02,01` | 2 pieces, first band 1 | two-piece descriptor with upper/front band split |
| `C4:2BBF` | `04,02` | 2 x 2 | first band is top row |
| `C4:2BE9` | `02,01` | 2 pieces, first band 1 | sibling of `C4:2BA9` |
| `C4:2BFF` | `04,02` | wider 2 x 2 | sibling of `C4:2BBF` |
| `C4:2C29` | `06,03` | 3 x 2 | first band is top row |
| `C4:2C67` | `06,04` | 3 x 2 tall/wide | first band is four pieces |
| `C4:2CA5` | `03,02` | 1 x 3 | first band is top two pieces |
| `C4:2CC5` | `06,04` | 2 x 3 or wide 3 x 2 | first band is four pieces |
| `C4:2D03` | `09,06` | 3 x 3 | first band is top two rows |
| `C4:2D5F` | `0C,08` | 4 x 3 | first band is top two rows |
| `C4:2DD9` | `10,08` | 4 x 4 | first band is top two rows |
| `C4:2E7B` | `14,08` | 4 x 5 | first band is top two rows |

These labels are deliberately structural. They are useful for source generation and debugger symbols, but the exact visible character/body-part names should wait until callers identify the displayed entity family.

`C4:2F8C` and `C4:303C` are adjacent support tables used by the same visual setup path:

- `C4:2F8C` is the VRAM tile-base offset ladder used when missing visual chunks are uploaded into the `#$4000/#$4100` ranges.
- `C4:303C` is the generated tile-word ladder used by `C0:1D38` when it rewrites runtime byte `1` and contributes the tile-number high bit to runtime byte `2`.

## Working Names

- `C4:2B51` = `SecondaryVisualDescriptor1Piece`
- `C4:2B5D` = `SecondaryVisualDescriptor2PieceNarrow`
- `C4:2B73` = `SecondaryVisualDescriptor2PieceWide`
- `C4:2B89` = `SecondaryVisualDescriptor3PieceColumn`
- `C4:2BA9` = `SecondaryVisualDescriptor2PieceBandSplit`
- `C4:2BBF` = `SecondaryVisualDescriptor4Piece2x2`
- `C4:2BE9` = `SecondaryVisualDescriptor2PieceBandSplitAlt`
- `C4:2BFF` = `SecondaryVisualDescriptor4Piece2x2Wide`
- `C4:2C29` = `SecondaryVisualDescriptor6Piece3x2`
- `C4:2C67` = `SecondaryVisualDescriptor6Piece3x2Tall`
- `C4:2CA5` = `SecondaryVisualDescriptor3Piece1x3`
- `C4:2CC5` = `SecondaryVisualDescriptor6Piece2x3Wide`
- `C4:2D03` = `SecondaryVisualDescriptor9Piece3x3`
- `C4:2D5F` = `SecondaryVisualDescriptor12Piece4x3`
- `C4:2DD9` = `SecondaryVisualDescriptor16Piece4x4`
- `C4:2E7B` = `SecondaryVisualDescriptor20Piece4x5`
- `C4:2F8C` = `VisualTileBaseVramOffsetTable`
- `C4:303C` = `VisualPieceTileWordLadder`
