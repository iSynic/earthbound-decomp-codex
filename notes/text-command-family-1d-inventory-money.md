...
- `0x10` = best current read: test whether an equipment slot/reference is currently selected (0 exposed parsed hits)
...
The front and middle local `0x1D` / adjacent item-helper strip is now checked in as decoded source in the C1 scaffold. The promoted source covers `C1:48AC..4CEE` for the compact item broad-class test, `0x1D 08/09`, the early HP/PP wrapper neighbors, and give/take item leaves; `C1:4CEE..4D93` for the inventory-room and negative item predicate leaves; `C1:4D93..4EAB` for the positive possession search, use-item, and teleport-preset leaves; `C1:4EF8..4F6F` for item buy/sell price wrappers; `C1:575D..621F` for equipped-item tests, Escargo storage transfer, wallet/ATM mutation/checks, party-count, storage queue, and random-number leaves; `C1:7058..711C` for the Escargo storage status classifier; `C1:7274..72BC` for the bank-deposit accumulator staging leaf; `C1:7708..776A` for the offensive/defensive equipment classifier; and `C1:8B2C..91F8` for the shared inventory insertion/removal, storage-capacity, equipment-slot, and pending-item queue helper layer.
...
One adjacent display bridge is now pinned too: `C1:5650` calls `C4:507A`, the active-window right-aligned decimal printer. That keeps the family split clean: the `0x1D` leaves mutate or test wallet/ATM/inventory state, while `C4:507A` is the shared number presentation helper that displays already-selected numeric values.
...
So the safest command-level reading is: test whether the selected character, or anyone in the active wildcard family, currently has the requested equipment reference selected. A higher-level script label may still collapse this to an equipped-item check, but the source contract should preserve the slot-reference shape.
