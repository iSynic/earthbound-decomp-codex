...
graphics, font data, an inferred SRAM mystery payload, a small generated
...
- `E0:09B4..E0:1358`: `COMPRESSED_SRAM`, inferred as `2469` bytes because
...
`E0:1FB9..E0:21A7` as one combined generated table span.
...
`mystery_sram.bin.lzhal` as missing payload metadata.
...
- UI graphics, fonts, inferred SRAM payload, generated text/town-map tables,
  town map graphics, and audio spans are exact for the US retail build.
...
- `src/e0/bank_e0_helpers_asar.asm` protects the full bank through the reusable
  ...
  byte-exact but still needs named asset metadata if we later emit individual
  source assets.

Still intentionally out of scope:

- Meaning and decompressed contents of `COMPRESSED_SRAM`.
- Internal splits inside the generated table span.
...
semantic: resolve the missing `mystery_sram.bin.lzhal` metadata if individual
asset extraction needs it, split the generated text-window/town-map table span
into typed contracts, and optionally add render fixtures for UI graphics, fonts,
and town maps.
