...
- The strongest current refinement on `C0:DB0F` is that the `$103E == 1` path now looks like a real ordered depth-sort or draw-order queue rather than a generic alternate dispatcher. The first pass routes those slots into the `$280C` chain, and the second pass repeatedly selects the minimum `$0BCA` entry before handing it to `C0:A0CA`.
...
- `Initialize_EntityWithSpritePose` at `C0:1E49` is now worth naming as the core entity-setup bridge inside that same layer. The safest current local read is that it takes a sprite-pose type in `A`, indexes the `EF:133F` pointer table, allocates the needed slot and record state, then seeds the per-entity visual/config families like `$2916/$2952/$298E/$2A06/$2A42/$2A7E/$2ABA/$2CD6` before returning. That is strong enough to treat `C0:1E49` as the main sprite-pose-driven entity initializer, even though the exact role of every seeded field is still being tightened.
- The allocator side of `C0:1E49` is now clearer too. `C0:1A9D` is the reference-named `FIND_FREE_7E4682` helper and behaves like a first-fit allocator over 5-byte records in `$467E..49FD`; `C0:1B15` releases those records; `C0:1B96` reserves contiguous bytes in the `$4A00..4A57` visual-memory ownership map; and `C0:1C11` rewrites or clears those reservations. The focused write-up lives in `notes/entity-pool-allocation-and-release-c01a9d-c020f1.md`.
- The placement side just past `Spawn_Entity` is clearer as well. `ebsrc-main` names `C0:2A6B` and `C0:2B55` as `SPAWN_HORIZONTAL` and `SPAWN_VERTICAL`; both call `C0:263D` as a `D0:1880` placement/tile-word lookup and require adjacent nonzero words to match before calling `C0:2668`. `C0:2668` chooses either a terrain-class-weighted special list or a table-backed candidate list from `D0:B880`, then funnels into the `C0:2A50` iterator and eventually `C0:1E49`. The focused write-up lives in `notes/entity-placement-probe-c0263d-c02668.md`.
- `Cleanup_EntitySlotState` at `C0:9C35` is also firmer now. The current best local model is: mark the slot inactive through `$0A62`, run `C0:9DA1` to restore record defaults, return any bound task-record chain through `C0:9C99`, detach the slot from the active chain through `C0:9C73`, and finally push the slot back to the free list through `C0:9C8F`. So the `2140 -> 9C35` tail now reads like a real full slot-release sequence, not just miscellaneous cleanup.
- `C0:20F1` is now separated from `C0:2140`: it releases the current slot id from `$1A42`, clears the `$467E` pool records and `$4A00` reservations, updates `$4A5C/$4A60`, and clears `$2CD6/$2C9A`, but does not call `C0:9C35`. `ebsrc-main` exposes it as `EVENT_UNKNOWN_C020F1`, so it looks like a script-facing partial release path rather than the full slot release.

- The battle and scene wrappers around this layer are now sharper. The reference-named `GET_ON_BICYCLE` at `C0:3C5E` frees slot `$0018`, sets bicycle/special traversal words (`$9887 = 6`, `$9883 = 3`), clears small scene-state words like `$9A0B/$987D/$0A38/$0A3A`, and calls `C0:1E49` with type `7`. The paired `C0:3CFD` path restores ordinary leader state by freeing slot `$0018`, clearing `$9887/$9883`, and calling `C0:1E49` with type `1`. The focused write-up lives in `notes/bicycle-transition-and-party-registry-c03c25-c03f1e.md`.
...
- `C0:329F` is now separated from the `$986F/$988B` registry rebuild cluster that follows it. It maps a character selector through `C0:8FF7` with stride `#$005F`, then clears seven bytes at `$99DC + offset`; the curated WRAM map identifies that span as party-character affliction/status bytes at record offset `+$0E..+$14`. The focused write-up lives in `notes/character-affliction-clear-c0329f.md`.

- The input/interaction seam after `CENTER_SCREEN` is also tighter now. `C0:402B` is a wrapper that installs the small `$007B/$007D/$007F/$0081/$0083` animation/script stream through `C0:83E3`; `C0:4049` clears that stream countdown; the reference-named `MAP_INPUT_TO_DIRECTION` at `C0:404F` maps `$0065` direction nibbles through `C3:E12C`; and `C0:4116` is the single-facing probe under the `C0:41E3 -> C0:4279` interaction resolver. The focused write-up lives in `notes/input-direction-and-interaction-probes-c0402b-c04116.md`.

- The next movement-tick seam is now mapped as well. `C0:5200`, installed by `EVENT_1` as a tick callback, calls `C0:4C45`, which is the main position/snapshot commit coordinator for `$9875/$9877/$9879/$987B/$987F/$9881/$9883/$9885` and the 12-byte snapshot records rooted at `$5156`.

- `C0:4C45` dispatches normal input movement through `C0:449B`, scripted mode `#$000C` through `C0:47CF`, bicycle/special traversal mode `#$0003` through `C0:48D3`, and temporary `$98A5` modes through `C0:4B53`. The focused write-up lives in `notes/position-snapshot-and-movement-tick-c0449b-c05200.md`.

- `EVENT_2` gives two more anchors in that same family: it calls `C0:4EF0` to restore the current slot from the snapshot record, then installs `C0:4D78` as an object/snapshot reconciliation tick callback. This helps connect the earlier `$5156` transition snapshot work from `C0:3F1E` to the live movement tick rather than treating those records as passive scratch.

- The local decoder gained 65C816 direct-page indirect opcodes during this pass, which matters for this exact cluster because `C0:4C45` and `C0:4D78` write snapshot records through `($dp)` and `($dp),Y` pointers.

- The next neighboring seam is now mapped too. `C0:52D4` seeds the `$5156` party trail/snapshot ring after transition-style setup, `C0:546B` sums active party character levels for a later threshold check, and `C0:54C9-C0:5E82` forms the collision/surface probe layer over the active 64x64 collision byte page at `$E000`.

- `Resolve_MovementSurfaceCollision` at `C0:5B7B` is now the front door below normal movement and special traversal probes. It stores candidate coordinates in `$5DAC/$5DAE`, samples collision bytes through `C0:5769` and the edge helpers at `5503/559C/5639/56D0`, and returns both a collision mask in `$5DA4` and a resolved surface/mode value in `$5DA6`.

- The focused write-up for this seam lives in `notes/collision-surface-probes-c052d4-c05e3a.md`. The decoder also learned opcode `FF` (`SBC long,X`) and opcode `25` (`AND dp`) so the footprint and terrain-compatibility helpers in this range decode cleanly.

- The short overlap seam immediately after it is now mapped as well. `C0:5F33` and `C0:5F82` are half-footprint collision probes, `C0:5FF6` performs a one-shot overlapping-entity search and stores the result in `$28CC`, and `C0:613C` / `C0:6267` update the persistent per-slot neighbor cache at `$289E[slot]`.

- `EVENT_UNKNOWN_C06478` is now grounded as a script-facing current-slot neighbor-cache refresh wrapper. It calls `C0:9F08` to derive `$2848/$284A` for `$1A42`, then runs the priority scanner at `C0:6267`. The focused write-up lives in `notes/entity-overlap-neighbor-cache-c05ece-c064d3.md`.

- A broad strip of small C0 utility routines is now cleared from the audit frontier as well. `C0:69F7` wraps the current-position `C0:68F4` music/area lookup, `C0:73C0` watches two movement-boundary rectangles and enqueues queue type `9`, `C0:778A` updates an event-driven current-slot offset around the leader, `C0:7A31` ORs the `$2E7A` overlay flag `#$4000`, and `C0:83B8-C0:88A5` covers input stream setup, input polling, transfer descriptor helpers, and frame/display wait utilities.

- The focused write-up for that utility strip lives in `notes/c0-small-utility-frontier-69f7-8c54.md`. During that pass the decoder learned several indexed direct-page opcodes and `EOR #imm`, which keeps the input poller at `C0:8496` from drifting.

- The next action-script/task frontier is now grounded too. `C0:8FC2` is table data inside the VRAM port triple table rather than executable code; `C0:9ABD-C0:9ADB` is the action-script AND/OR/ADD/EOR mutation mini-dispatch; `C0:9B09-C0:9BF8` covers compact script variable, branch, wait, and callback installers; and `C0:9CD7-C0:9ECE` extends the task-slot allocator model through free-list compaction, record-chain walking, script-facing task constructors, and bulk release wrappers.

- The focused write-up for that seam lives in `notes/actionscript-dispatch-task-frontier-c08fc2-c09ece.md`. During that pass the decoder learned opcode `45` (`EOR dp`) so the fourth mini-dispatch handler at `C0:9ADB` decodes without drifting.

- The following task/callback strip is mapped in `notes/task-freeze-position-callbacks-maplookup-c09f3b-c0a26b.md`. It covers the event-callable task freeze/restore wrappers at `C0:9F3B` and `C0:9F71`, the random/fade action-script helpers at `C0:9F82-C0:9FBB`, movement integration and world-to-screen callbacks at `C0:9FC8-C0:A0BB`, the data-pointer callback dispatcher at `C0:A0CA/C0:A0E3`, and the cached packed-map/property lookup at `C0:A156` with its `C0:A1AE` shift-entry table.

- The adjacent physics/sprite field strip is mapped in `notes/physics-sprite-field-frontier-c0a317-c0a6cb.md`. It names the comparison callbacks selected by `C0:A350`, the gated position refresh callbacks at `C0:A360` and `C0:A384`, the display callback body at `C0:A3A4`, the profile refresh family through `C0:A56E`, and the compact `C0:A643-C0:A6CB` event helpers that get/set current-slot fields such as `$2AF6`, `$2B32`, `$289E`, `$28DA`, and `$2C5E`.

- The next action-script wrapper strip is mapped in `notes/actionscript-wrapper-strip-c0a841-c0aafd.md`. It covers `C0:A841-C0:AAFD` as the event/action-script ABI run that feeds sound/effect queues, C2/C4 entity/effect helpers, the `ACTIONSCRIPT_PREPARE_NEW_ENTITY*` family, `MOVEMENT_LOAD_BATTLEBG`, `ACTIONSCRIPT_FADE_OUT_WITH_MOSAIC`, and the `AAD5/AAFD` per-script-frame countdown field.

- The following hardware-facing frontier is mapped in `notes/audio-apu-battlebg-transfer-frontier-c0ab06-c0ae44.md`. It covers the SPC700 upload handshake, APUIO music/sound command wrappers at `C0:ABBD-C0:AC3A`, the `STEREO_MONO_DATA` table, and the battle-background descriptor/DMA setup family through `C0:AE44`.

- The adjacent PPU colour/window frontier is mapped in `notes/ppu-colour-window-bg-offset-frontier-c0afcd-c0b3ff.md`. It covers battle-background colour math presets at `C0:AFCD`, `SET_COLDATA`, `SET_COLOUR_ADDSUB_MODE`, `SET_WINDOW_MASK`, the window-position DMA helpers at `C0:B0B8/B0EF`, and the `$3FD0` BG offset table generator through the `C0:B2FF-C0:B3FF` lookup data.

- The intro overworld setup pair is mapped in `notes/intro-overworld-position-init-c0b65f-c0b67f.md`. `C0:B65F` seeds player X/Y/facing fields, and `C0:B67F` initializes the intro/Onett pan scene before control reaches the broader named main-loop region.

- The pathfinding lead-in is mapped in `notes/pathfinding-frontier-c0b9bc-c0ba35.md`. `C0:B9BC` snapshots live entity positions into wrapped 64x64 path-grid coordinates, while `C0:BA35` builds the `$7F3000` occupancy grid, extracts candidate entity records, and calls the C4 path solver before the named `FIND_PATH_TO_PARTY` region.

- The adjacent path consumer and direction helper strip is mapped in `notes/pathfinding-consumers-direction-helpers-c0bd96-c0c7db.md`. `C0:BD96` and `C0:BF72` prepare one-target `$F200` path requests, `C0:C0B4/C19B/C251` copy path buffers into `$4A96` lanes, `C0:C4F7-C0:C69E` covers player-relative direction choices, and `C0:C6B6-C0:C7DB` adds proximity, alignment, and footprint gates for the current slot.

- The following action-script movement-vector runtime is mapped in `notes/movement-vector-script-runtime-c0c83b-c0d195.md`. `C0:C83B` installs cardinal/diagonal velocity words from the script direction and `$2B32` speed field, `C0:CA4E/CBD3/CC11` derive task timers into `$1372`, `C0:CCCC/CD50/CEBE` manage arc/phase movement state through `$0F4E/$0F8A/$2DC6`, and `C0:CF97/D0D9/D0E6/D15C` provide collision-map target search and movement predicates for wandering scripts.

- The NPC attention/path coordinator is mapped in `notes/npc-attention-path-coordinator-c0d19b-c0d98f.md`. `C0:D19B` builds a focal-slot relation/path set around `$4DB6/$4DB8`, `C0:D5B0` starts or advances that coordinator from event scripts, `C0:D77F/D7B3/D7C7/D7E0` provide cleanup and save/restore helpers, and `C0:D7F7/D98F` consume path steps into live movement or `$0FC6/$1002` target coordinates.

- The teleport freeze/vector frontier is mapped in `notes/teleport-freeze-vector-frontier-c0dd0f-c0e196.md`. `C0:DD0F/DD2C` are frame-pump waits, reference-named `SET_TELEPORT_STATE` at `C0:DD53` seeds `$9F3F/$9F41`, `C0:DE16/DE46/DE7C` flag and unflag transition object slots, `C0:DED9/DF22` provide footprint/vector helpers, and `C0:E196` snapshots live player state into the `$5156` ring.

- The larger teleport dispatcher is mapped in `notes/teleport-mainloop-state-machines-c0e214-c0ebdf.md`. Reference-named `TELEPORT_MAINLOOP` at `C0:EA99` dispatches `$9F41` styles into `C0:E28F` alpha/straight movement, `C0:E516` beta/curved movement, `C0:E674/E776` success-drift callbacks, `C0:E3C1/E6FE/E97C` restore/refresh callbacks, and `C0:E815/E897/E9BA` success/failure setup paths before restoring normal overworld callbacks.

- The post-teleport title/intro tail is split between `notes/title-screen-logo-palette-event-helpers-c0ebe0-c0ee53.md` and `notes/intro-logo-wait-and-gas-station-helpers-c0efe1-c0f21e.md`. `C0:EBE0` loads title-logo graphics/tilemaps, `C0:EC77-C0:EDDA` are title palette/frame event helpers, `C0:EE47/EE53` touch intro display/entity flags, `C0:EFE1` is a cancelable frame wait, and `C0:F1D2/F21E` cover the gas-station intro fade and animated screen loop.

- The final audit residuals are symbol/reference artifacts rather than unstudied chunks. `C0:035B` is ebsrc's `UNUSED_C0035B`, a small `$7F:8000` table reader indexed by `A`, `X`, and `Y`; `C0:9622` is the inner load-and-branch entry of action-script `MOVEMENT_CODE_24` and is documented with the action-script interpreter note; `C0:ED41` is present as a JP event symbol alias, while the US routine start is already covered at `C0:ED39`.
...
- Tighten the interpretation of `Dispatch_ActiveTaskSlots` at `C0:DB0F` a little farther. The `$103E == 1` path now has a good local case as an ordered depth-sort or draw-order queue, but the exact consumer-side meaning of that second pass still deserves one more read.
