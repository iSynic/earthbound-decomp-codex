...
The early `C1:078D..0F40` strip is now source-backed through the durable C1 scaffold. The C1 side is much clearer: descriptor-backed window tilemap construction, HP/PP display window sync, glyph placement, side-effectful letter print, decimal staging, decimal printing, and fixed-string printing. The adjacent `C1:0F40..17E2` text-entry/window-record corridor is also source-backed; the latest polish passes name its clear-window, glyph reset, instant-print, decimal-format, active-chain rendering, title-upload, text-length, sound, input-tick, multiply, and divide helper joins.
