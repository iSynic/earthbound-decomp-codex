...
- `tools/render_map_scene_metatile_previews.py` writes ignored `build/map-scene-metatile-previews/` grayscale sector previews by composing `map_tiles.map` tile IDs through `.fts` arrangement/collision records and tile-pixel rows.
...
- Map scene metatile previews can now be generated locally for direct `.fts` sectors. The renderer composes each scene's 8x8 map-tile window into a 256x256 grayscale sector image through the audited 4x4 arrangement cells and 8x8 tile-pixel rows.
