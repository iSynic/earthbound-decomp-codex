...
- Map `.fts` animation/settings contracts now cover `168` variable rows across the `20` direct exports. Each row has a unique two-character base32-like row ID and splits into five 58-character blocks. The contract now ties the rows to WRAM `$43DC..$445B`, EF tile-animation pointer/property refs, and the `20` known `MAP_DATA_TILE_ANIMATION_GFX_N` payloads, with all `32` observed row groups owned by a single direct `.fts` export.
...
2. Tile animation/settings row field decoding against EF property pointer tables and runtime `$43DC..$445B` consumers.
