...
- `tools/build_map_sprite_usage_contract.py` builds `notes/map-sprite-usage-contract.json`, a reference-backed join from map sprite placements and NPC config rows to overworld sprite enum IDs and animation-role metadata.
...
- Map sprite usage contracts now join `1584` NPC config rows and `1582` map placements across `627` populated sectors to overworld sprite metadata. The join sees `297` unique NPC sprite IDs: `163` have full animation-role contracts and `134` currently fall back to enum plus `sprite_groups.yml` length/size metadata.
