...
- `tools/build_map_fts_animation_settings_contract.py` builds `notes/map-fts-animation-settings-contract.json`, a structural audit for the variable `.fts` 290-character tile-animation/settings rows, including ref anchors, row group ownership, and block-position profiles.
...
- Map `.fts` animation/settings contracts now cover `168` variable rows across the `20` direct exports. Each row has a unique two-character base32-like row ID and splits into five 58-character blocks. The contract now ties the rows to WRAM `$43DC..$445B`, the ROM-verified EF animation runtime table spans, and the `20` known `MAP_DATA_TILE_ANIMATION_GFX_N` payloads, with all `32` observed row groups owned by a single direct `.fts` export.
...
2. Field decoding for the separate 290-character `.fts` animation/settings rows, now anchored against the ROM-verified C0/EF animation runtime contract.
