...
- `tools/build_map_fts_animation_settings_contract.py` builds `notes/map-fts-animation-settings-contract.json`, a structural audit for the variable `.fts` 290-character tile-animation/settings rows.
...
- Map palette descriptor context audits now show each bank DA map palette variant is `192` bytes (`96` colors, six 16-color subpalettes), while live arrangement descriptors use palette values `0-7`. Raw descriptor interpretation overflows `266387` of `972800` scene cells (`27.384%`), so palette-aware previews deliberately record overflow instead of hiding this unresolved mapping.
...
- Map scene metatile previews can now be generated locally for direct `.fts` sectors. The renderer composes each scene's 8x8 map-tile window into a 256x256 grayscale sector image through the audited 4x4 arrangement cells and 8x8 tile-pixel rows. Palette-aware preview mode now resolves each scene's tileset/palette variant through the bank DA map palette assets and records descriptor-palette overflow counts where the three-bit arrangement palette field exceeds the six 16-color subpalettes present in one 192-byte map palette variant.
...
1. Runtime corroboration for the map descriptor palette-to-subpalette mapping that remains unresolved after the DA pointer-table contract.
