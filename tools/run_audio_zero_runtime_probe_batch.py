#!/usr/bin/env python3
"""Run or dry-run targeted 0x00 runtime probe jobs."""
...
import audio_zero_runtime_probe_stub_harness
import validate_audio_zero_runtime_probe_result


ROOT = Path(__file__).resolve().parent.parent
DEFAULT_JOBS = ROOT / "build" / "audio" / "zero-runtime-probe-jobs" / "zero-runtime-probe-jobs.json"
DEFAULT_PLAN = ROOT / "manifests" / "audio-zero-runtime-probe-plan.json"
DEFAULT_SUMMARY = ROOT / "build" / "audio" / "zero-runtime-probe-jobs" / "zero-runtime-probe-batch-summary.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run or dry-run audio 0x00 runtime probe jobs.")
    parser.add_argument("--jobs", default=str(DEFAULT_JOBS), help="Generated zero-runtime probe job index path.")
    parser.add_argument("--plan", default=str(DEFAULT_PLAN), help="Committed zero-runtime probe plan for validation.")
    parser.add_argument(
        "--mode",
        default="dry-run-stub",
        choices=["dry-run-stub", "external"],
        help="Run schema-only dry runs or invoke an external zero-probe harness.",
    )
...
def validate_result(result_path: Path, plan_jobs: dict[str, dict[str, Any]]) -> None:
    result = validate_audio_zero_runtime_probe_result.load_json(result_path)
    ...
    if errors:
        ...
        raise ValueError(f"zero-runtime probe result validation failed:\n{formatted}")


def write_dry_run_result(job: dict[str, Any]) -> Path:
    result_path = repo_path(str(job["result_path"]))
    result_path.parent.mkdir(parents=True, exist_ok=True)
    result = audio_zero_runtime_probe_stub_harness.build_result(
        job,
        harness_version="zero-runtime-probe-batch-dry-run",
        status="unsupported",
    )
...
def run_external(job: dict[str, Any], job_path: Path, command: list[str] | None) -> Path:
    ...
    if result.returncode:
        raise RuntimeError(f"external zero-probe harness exited with status {result.returncode}: {expanded}")
...
def write_summary(index: dict[str, Any], summary_path: Path, batch: dict[str, Any]) -> None:
    summary = {
        "schema": "earthbound-decomp.audio-zero-runtime-probe-batch-run.v1",
        "job_index": str(summary_path.parent / Path(str(index.get("runner", {}).get("job_index_path", "zero-runtime-probe-jobs.json"))).name),
        "mode": batch["mode"],
...
def main() -> int:
    ...
    print(f"Selected {len(selected)} zero-runtime probe jobs for {args.mode} mode")
    for job in selected:
