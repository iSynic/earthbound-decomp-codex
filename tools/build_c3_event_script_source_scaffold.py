...
def render_scaffold(spans: list[Span], gaps: list[Gap], rom: bytes, *, start: int, end: int) -> tuple[str, dict[str, int]]:
    ...
    lines = [
        ...
        ";",
        "; This is intentionally separate from bank_c3_helpers_asar.asm while the",
        "; include strategy is being proven. It assembles all validated C3",
        "; event/actionscript source pilots plus preserved raw gaps for",
...
def write_report(path: Path, manifest: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "This scaffold is a graduation harness: it assembles every validated C3 event/actionscript source pilot together with the raw bytes that are not event/actionscript candidates. It is separate from `src/c3/bank_c3_helpers_asar.asm` until the main bank include plan is proven end to end.",
        "",
        "## Preserved Gaps",
        "",
        "| Range | Bytes | SHA1 |",
