...
def build_record(probe_root: Path, summary_path: Path, minimums: dict[str, list[str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "watch_snapshot_counts": summary.get("watch_snapshot_counts", {}),
...
def render_note(manifest: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- `c1_c2_target_action_staging` now has separate routes for target setup, item-action resolution, inventory-selection loops, controlled `C2:B930` mechanics, and a natural Fresh Egg Goods Use `C1:B3DB -> C2:B930` snapshot-export proof. Additional item/target families are optional broadening, not initial proof blockers.",
            ...
            "- `resource_amount_pair_magnet_vs_pp_loss` now has controlled reducer probes plus natural save-state captures that separate PSI Magnet transfer routing from Mad Duck loss-only PP reduction. PSI Magnet reaches `C2:9F5E -> C2:721D -> C2:7191` with target PP loss and capped caster recovery routing; Mad Duck reaches `C2:8E42 -> C2:721D -> C2:7191` with no caster-side recovery call. Remaining work is source/status polish and optional broader coverage.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
