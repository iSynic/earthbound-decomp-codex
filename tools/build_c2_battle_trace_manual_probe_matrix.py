...
def render_note(manifest: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- `resource_amount_pair_magnet_vs_pp_loss` now has fixture-steered entry hits for both `C2:9F5E` and `C2:8E42`, controlled WRAM-patched reducer probes that separate PSI Magnet transfer mechanics from PP reduction loss-only mechanics, and autonomous scripted-entry enemy fixtures that reach canonical Gigantic Ant row 54 and Guardian General row 95 lanes. Natural unpatched PP-bearing target traces remain pending before proof-grade promotion.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
