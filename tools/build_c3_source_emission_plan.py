...
def render_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            + ". The remaining C3 source work is broader build-candidate hardening: keeping the source prototypes, generated `.bytes.asar.asm` companions, durable scaffold, and adjacent visual/script data assets in byte-equivalent lockstep.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
