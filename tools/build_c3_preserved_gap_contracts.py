...
GROUPS: list[dict[str, Any]] = [
    {
        ...
        "segments": [
            ...
            },
            {
                ...
                "source_expectation": "ordinary 65816 helper; promoted into the event/actionscript source-pilot scaffold as supplemental native source",
                "evidence": [
                    "notes/c4-system-error-screen-render-0b51-0b75.md:55",
...
def build_manifest(rom: bytes) -> dict[str, Any]:
    ...
    source_helper_bytes = sum(
        segment["size"]
        for group in groups
        for segment in group["segments"]
        if segment["classification"] == "source-helper"
    )
    return {
        "schema": SCHEMA,
        ...
        "summary": {
            ...
            "contract_region_bytes": total_bytes,
            "source_helper_bytes": source_helper_bytes,
            "raw_preserved_bytes": total_bytes - source_helper_bytes,
...
def write_report(path: Path, manifest: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "This report explains the C3 prefix/mid/late contract regions around the event/actionscript source-pilot scaffold. They are not unclaimed bytes: each region is covered by checked-in source data, refs, and a byte hash. Source-helper slices may graduate out of the raw preserved gaps while the surrounding data stays preserved by contract.",
        "",
        "## Summary",
        "",
        f"- preserved groups: `{summary['preserved_groups']}`",
        f"- contract-region bytes: `{summary['contract_region_bytes']}`",
        f"- source-helper bytes: `{summary['source_helper_bytes']}`",
        f"- raw preserved bytes: `{summary['raw_preserved_bytes']}`",
...
def main() -> int:
    ...
    print(
        ...
        f"{manifest['summary']['raw_preserved_bytes']} raw preserved bytes"
    )
    return 0
