...
def byte_listing_path_for_source(source_path: str | Path) -> Path:
    path = Path(source_path)
    if path.name.endswith(".bytes.asar.asm"):
        return path
    if path.suffix.lower() == ".asm":
        return path.with_name(f"{path.name[:-4]}.bytes.asar.asm")
    return path.with_name(f"{path.name}.bytes.asar.asm")
...
def count_literal_db_bytes(lines: list[str]) -> int:
    count = 0
    for line in lines:
        code = strip_comment(line).strip()
        if re.match(r"^(db|\.byte)\b", code, re.IGNORECASE):
            count += len(re.findall(r"\$[0-9A-F]{2}\b", code, re.IGNORECASE))
    return count
...
def source_label_addresses(source_lines: list[str], bank: int) -> set[int]:
    addresses: set[int] = set()
    bank_prefix = f"{bank:02X}"
    for line in source_lines:
        stripped = strip_comment(line).strip()
        label = LABEL_RE.match(stripped)
        if not label:
            continue
        address = re.match(rf"^{bank_prefix}([0-9A-F]{{4}})_", label.group(1), re.IGNORECASE)
        if address:
            addresses.add(int(address.group(1), 16))
    return addresses


def segment_label_addresses(segment: dict[str, Any]) -> set[int]:
    addresses: set[int] = set()
    for raw in segment.get("labels", []):
        match = re.match(r"^([C-E][0-9A-F]):([0-9A-F]{4})\b", str(raw), re.IGNORECASE)
        if match:
            addresses.add(int(match.group(2), 16))
    return addresses


def source_mentions_protected_gap(source_lines: list[str], segment: dict[str, Any]) -> bool:
    range_text = f"{segment['start']}..{segment['end']}".lower()
    for line in source_lines:
        if range_text in line.lower() and "data gap" in line.lower():
            return True
    return False


def missing_source_segment_bytes(
    module: dict[str, Any],
    rom: bytes,
    label_addresses: set[int],
    source_lines: list[str],
) -> dict[int, bytes]:
    gaps: dict[int, bytes] = {}
    for segment in module.get("source_segments", []):
        segment_addresses = segment_label_addresses(segment)
        if segment_addresses & label_addresses:
            continue
        if not segment_addresses:
            continue
        if not source_mentions_protected_gap(source_lines, segment):
            continue
        start_bank, start_address = parse_bank_address(segment["start"])
        end_bank, end_address = parse_bank_address(segment["end"])
        if start_bank != end_bank:
            raise ValueError(f"Source segment crosses banks: {segment['start']}..{segment['end']}")
        start_offset = hirom_to_file_offset(start_bank, start_address, len(rom))
        end_offset = hirom_to_file_offset(end_bank, end_address, len(rom))
        if start_offset is None or end_offset is None:
            raise ValueError(f"Unable to convert source segment {segment['start']}..{segment['end']} to ROM offsets")
        gaps[end_address] = rom[start_offset:end_offset]
    return gaps
...
def translate_source_to_asar(
    ...
    *,
    ...
    label_addresses = source_label_addresses(source_lines, start_bank)
    gaps_by_next_label = data_gap_bytes(module, rom)
    if count_literal_db_bytes(source_lines) < int(module.get("source_size", 0)):
        for end_address, gap_data in missing_source_segment_bytes(module, rom, label_addresses, source_lines).items():
            gaps_by_next_label.setdefault(end_address, gap_data)
...
def render_combined_scaffold(
    ...
    *,
    ...
    for module in modules:
        ...
        if split_sources:
            byte_path = byte_listing_path_for_source(source_path)
            chunks.append(f'; Source: {rel(source_path)}')
            chunks.append(f'incsrc "{byte_path.name}"')
            chunks.append("")
        else:
            chunks.append(
    ...
    for module in modules:
        ...
        byte_path.write_text(
            translate_source_to_asar(
                ...
                rom,
                generated_by=generated_by,
                module_purpose="Byte-preserving Asar listing for this source unit.",
            ),
            encoding="utf-8",
        )
        written.append(byte_path)
    return written
