...
def joinable_experiment(experiment: dict[str, Any]) -> bool:
    diff = experiment.get("diff")
    return (
        experiment.get("evidence_level") == "diff-confirmed"
        and isinstance(diff, dict)
        and diff.get("status") == "different"
        and isinstance(diff.get("first_changed_offset"), str)
    )


def unjoined_reason(experiment: dict[str, Any]) -> str:
    diff = experiment.get("diff")
    if experiment.get("dry_run"):
        return "dry-run-no-rom-diff"
    status = experiment.get("experiment_status")
    if isinstance(status, str) and status:
        return status
    if not isinstance(diff, dict):
        return "missing-diff"
    if diff.get("status") != "different":
        return f"diff-status-{diff.get('status', 'unknown')}"
    if not isinstance(diff.get("first_changed_offset"), str):
        return "missing-first-changed-offset"
    return "not-diff-confirmed"


def pending_experiment(experiment_id: str, experiment: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "reason": unjoined_reason(experiment),
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    if report.get("pending_experiments"):
        pending_lines.extend(
            [
                "## Pending Experiments",
                "",
                "| Experiment | CoilSnake file | Evidence | Reason |",
                "| --- | --- | --- | --- |",
            ]
        )
        for experiment in report["pending_experiments"]:
            pending_lines.append(
                "| {experiment} | `{file}` | `{evidence}` | `{reason}` |".format(
                    experiment=experiment["experiment_id"],
                    file=experiment.get("coilsnake_file") or "unknown",
                    evidence=experiment.get("evidence_level") or "unknown",
            ...
            pending_experiments.append(
                {
                    "experiment_id": experiment_id,
                    "join_status": "not-joined",
                    "reason": "malformed-experiment-record",
                }
            )
            continue
