    ...
    return "\n".join(
        [
            ...
            *watch_lines,
            ...
            "    end",
            "    if addr ~= nil and #bytes > 0 then patches[#patches + 1] = { address = addr, bytes = bytes, source = trim(segment) }",
            ...
            "    local before = readBytes(patch.address, #patch.bytes)",
            "    for i, value in ipairs(patch.bytes) do wb((patch.address + i - 1) % 0x10000, value) end",
            "    writeJson({ type = \"wram_patch\", oracleId = oracleId, jobId = jobId, scenarioName = scenarioName, index = index, reason = reason, address = hex(patch.address), bytes = #patch.bytes, beforeHex = before, afterHex = readBytes(patch.address, #patch.bytes), source = patch.source })",
            "  end",
            "end",
            ...
            "      local before = readBytes(patch.address, #patch.bytes)",
            "      for i, value in ipairs(patch.bytes) do wb((patch.address + i - 1) % 0x10000, value) end",
            "      writeJson({ type = \"wram_patch\", oracleId = oracleId, jobId = jobId, scenarioName = scenarioName, index = index, reason = \"on_hit:\" .. label, address = hex(patch.address), bytes = #patch.bytes, beforeHex = before, afterHex = readBytes(patch.address, #patch.bytes), source = patch.source })",
            "    end",
            "  end",
            "end",
            ...
            "writeJson({ type = \"runner_start\", oracleId = oracleId, jobId = jobId, scenarioName = scenarioName, jobPath = jobPath, runnerVersion = runnerVersion, requiredHitCount = requiredHitCount, inputPattern = inputPatternSpec, inputPatternFrames = inputPatternFrames, wramPatchTiming = wramPatchTiming })",
            "",
