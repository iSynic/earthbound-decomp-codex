...
def validate(data: dict[str, Any]) -> None:
    ...
    for job in jobs:
        ...
        require(int(tail.get("last_nonzero_sample_index", -1)) >= int(tail.get("finite_end_sample", 0)), f"{job_id}: bad tail ordering")
        require(int(tail.get("rendered_samples", 0)) > int(tail.get("finite_end_sample", 0)), f"{job_id}: render should extend past candidate")
