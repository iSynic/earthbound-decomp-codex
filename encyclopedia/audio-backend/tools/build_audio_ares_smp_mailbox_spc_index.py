#!/usr/bin/env python3
"""Build a snes_spc-compatible snapshot index from the ares SMP mailbox smoke."""
...
DEFAULT_OUTPUT = ROOT / "build" / "audio" / "ares-smp-mailbox-spc" / "ares-smp-mailbox-spc-snapshots.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build ares SMP mailbox SPC snapshot index.")
    parser.add_argument("--summary", default=str(DEFAULT_SUMMARY), help="ares SMP mailbox smoke summary JSON.")
...
def main() -> int:
    ...
    for record in summary.get("records", []):
        ...
        capture_path = absolutize(str(record["result_path"]), base=ROOT)
        if not snapshot:
    ...
    index = {
        ...
        "snapshot_kind": "ares_smp_mailbox_last_keyon_spc_snapshot",
        "faithfulness": "ares_smp_object_with_timed_real_smp_portwrite_at_command_read_boundary",
    ...
    print(
        "Built ares SMP mailbox SPC index: "
