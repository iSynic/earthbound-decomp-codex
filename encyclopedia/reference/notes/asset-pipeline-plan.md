...
- `tools/build_asset_data_contract_frontier.py` consumes checked-in asset manifests and contract docs to write `notes/asset-data-contract-frontier.md` plus ignored `build/asset-data-contract-frontier.json`, separating contract-backed asset families from manifest-only frontiers and distinguishing manifest-inferred payload metadata from unresolved missing metadata.
...
- `tools/build_text_window_skin_bundle_contracts.py` consumes the local E0 text-window property table bytes plus ignored EBDecomp `WindowGraphics` refs to write `notes/text-window-skin-bundle-contracts.md` and ignored `build/text-window-skin-bundle-contracts.json`.
- `tools/build_font_bundle_contracts.py` consumes E0/E1 font manifests, local user-ROM-derived font metric/graphics outputs, ignored EBDecomp font refs, and the legacy main-font character map to write `notes/font-bundle-contracts.md` and ignored `build/font-bundle-contracts.json`.
- `tools/build_intro_title_visual_bundle_contracts.py` consumes the E1 manifest, local user-ROM-derived intro/title visual outputs, ignored EBDecomp logo/title refs, and legacy compressed tilemap/palette refs to write `notes/intro-title-visual-bundle-contracts.md` and ignored `build/intro-title-visual-bundle-contracts.json`.
- `tools/build_title_screen_palette_animation_contracts.py` consumes the current E1:AE7C manifest asset output, splits it using legacy labels, and writes `notes/title-screen-palette-animation-contracts.md` plus ignored `build/title-screen-palette-animation-contracts.json`.
- `tools/build_title_screen_letter_oam_contracts.py` consumes the local E1:CE08 table bytes, EBDecomp title-character position refs, and legacy title-letter OAM labels to write `notes/title-screen-letter-oam-contracts.md` and ignored `build/title-screen-letter-oam-contracts.json`.
- `tools/build_landing_cast_visual_contracts.py` consumes the E1 manifest, local user-ROM-derived landing/cast outputs, C4 runtime-owner notes, ebsrc include order, and ignored EBDecomp visual refs to write `notes/landing-cast-visual-contracts.md` and ignored `build/landing-cast-visual-contracts.json`.
- `tools/build_sram_template_contracts.py` consumes the E0 compressed SRAM manifest entry, local user-ROM-derived decompressed template, and ebsrc SRAM/save-block structs to write `notes/sram-template-contracts.md` and ignored `build/sram-template-contracts.json`.
...
- `tools/build_psi_animation_bundle_contracts.py` consumes the locally extracted CC PSI config/pointer table bytes plus the CC manifest and ebsrc loader evidence to write `notes/psi-animation-bundle-contracts.md` and ignored `build/psi-animation-bundle-contracts.json`.
- `tools/build_battle_sprite_bundle_contracts.py` consumes the CD/CE manifests, ebsrc battle sprite pointer table, and ebsrc enemy config rows to write `notes/battle-sprite-bundle-contracts.md` and ignored `build/battle-sprite-bundle-contracts.json`.
- `tools/build_swirl_sequence_bundle_contracts.py` consumes the CE manifest, ebsrc swirl pointer table, local user-ROM-derived swirl table bytes, and ignored EBDecomp swirl metadata to write `notes/swirl-sequence-bundle-contracts.md` and ignored `build/swirl-sequence-bundle-contracts.json`.
