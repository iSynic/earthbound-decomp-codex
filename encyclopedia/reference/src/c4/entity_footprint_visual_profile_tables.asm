...
;
; The footprint tables are consumed by movement/collision probes, while the
; secondary visual descriptor rows are consumed by C0 visual setup. C4 owns
; these row layouts; keep labels table-local unless a caller proves a broader
; engine-wide field name.
...
; Secondary visual descriptor records: byte 0 = pieces per pass, byte 1 =
; first priority-band count. Each piece is a 5-byte record, and record byte 4
; uses bit 7 on the final piece of each body pass.
...
C42F45_SetPartyTickCallbacks_PreservedBytes:
    ; Preserved adjacent code island from the descriptor contract note. It is
    ; byte-owned here but still intentionally emitted as data in this corridor.
