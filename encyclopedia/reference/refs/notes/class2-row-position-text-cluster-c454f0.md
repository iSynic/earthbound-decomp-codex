...
This note captures the small battle-text cluster that begins immediately after `C4:51FA` and helps anchor the late side-token path in player-visible row-position text.
...
It calls `C4:3E31` with length `#$001E` while walking a pointer into the same post-`C451FA` region. That `0x001E` length matches the size of the whole cluster from `C4:54F0` through `C4:550D` exactly.
